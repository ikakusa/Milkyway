...
static ID3D12Resource* resourceBuffer = nullptr;
...
bool ImFX::NewFrame(ID3D12Device* d3d12Device, ID3D12Resource* dxgiBackBuffer, float fxdpi)
{
...
void BeginFX(const ImDrawList* parent_list, const ImDrawCmd* cmd)
{
    ...
    auto rect = D2D1::RectU(0, 0, (UINT32)io.DisplaySize.x, (UINT32)io.DisplaySize.y);
...
void EndFX(const ImDrawList* parent_list, const ImDrawCmd* cmd)
{
    ...
    if (currentContext.back().targetBitmap != nullptr) currentContext.back().targetBitmap->Release();
...
ID2D1PathGeometry* CreateRoundRect(int x, int y, int width, int height, int leftTop, int rightTop, int rightBottom, int leftBottom)
{
    ...
    p[0].x = (FLOAT)x + (FLOAT)leftTop;
    p[0].y = (FLOAT)y;
    sink->BeginFigure(p[0], D2D1_FIGURE_BEGIN::D2D1_FIGURE_BEGIN_FILLED);
    p[1].x = (FLOAT)x + (FLOAT)width - (FLOAT)rightTop;
    p[1].y = (FLOAT)y;
    sink->AddLines(p, 2);

    p[0].x = (FLOAT)x + (FLOAT)width;
    p[0].y = (FLOAT)y + (FLOAT)rightTop;

    if (rightTop)
    {
        D2D1_POINT_2F p2 = D2D1::Matrix3x2F::Rotation(0, p[1]).TransformPoint(p[0]);
        sink->AddArc(D2D1::ArcSegment(p2, D2D1::SizeF((FLOAT)rightTop, (FLOAT)rightTop), 0, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_SMALL));
    }

    p[1].x = (FLOAT)x + (FLOAT)width;
    p[1].y = (FLOAT)y + (FLOAT)height - (FLOAT)rightBottom;
    sink->AddLines(p, 2);

    p[0].x = (FLOAT)x + (FLOAT)width - (FLOAT)rightBottom;
    p[0].y = (FLOAT)y + (FLOAT)height;

    if (rightBottom)
    {
        D2D1_POINT_2F p2 = D2D1::Matrix3x2F::Rotation(0, p[1]).TransformPoint(p[0]);
        sink->AddArc(D2D1::ArcSegment(p2, D2D1::SizeF((FLOAT)rightBottom, (FLOAT)rightBottom), 0, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_SMALL));
    }

    p[1].x = (FLOAT)x + (FLOAT)leftBottom;
    p[1].y = (FLOAT)y + (FLOAT)height;
    sink->AddLines(p, 2);

    p[0].x = (FLOAT)x;
    p[0].y = (FLOAT)y + (FLOAT)height - (FLOAT)leftBottom;
    if (leftBottom)
    {
        D2D1_POINT_2F p2 = D2D1::Matrix3x2F::Rotation(0, p[1]).TransformPoint(p[0]);
        sink->AddArc(D2D1::ArcSegment(p2, D2D1::SizeF((FLOAT)leftBottom, (FLOAT)leftBottom), 0, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_SMALL));
    }


    p[1].x = (FLOAT)x;
    p[1].y = (FLOAT)y + (FLOAT)leftTop;
    sink->AddLines(p, 2);
    p[0].x = (FLOAT)x + (FLOAT)leftTop;
    p[0].y = (FLOAT)y;
    if (leftTop)
    {
        D2D1_POINT_2F p2 = D2D1::Matrix3x2F::Rotation(0, p[1]).TransformPoint(p[0]);
        sink->AddArc(D2D1::ArcSegment(p2, D2D1::SizeF((FLOAT)leftTop, (FLOAT)leftTop), 0, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_SMALL));
    }
...
void ColorCallback(const ImDrawList* parent_list, const ImDrawCmd* cmd) {
    ...
    ID2D1PathGeometry* roundedRectGeo = CreateRoundRect((int)clipRect.x, (int)clipRect.y, (int)clipRect.z - (int)clipRect.x, (int)clipRect.w - (int)clipRect.y, (int)rounding.x, (int)rounding.y, (int)rounding.z, (int)rounding.w);
