...
__forceinline double animate(double endPoint, double current, double speed) {
	speed = max(0.0, min(1.0, speed));  // 0 ~ 1
	double diff = (double)fabs(endPoint - current);
	double o = diff * speed;
	return current + (endPoint > current ? o : -o);
}
__forceinline float animate(float endPoint, float current, float speed) {
	speed = max(0.f, min(1.f, speed));  // 0 ~ 1
	float diff = (float)fabs(endPoint - current);
	float o = diff * speed;
	return current + (endPoint > current ? o : -o);
}

template <typename T>
__forceinline static T getRandom(T min, T max) {
	static_assert(std::is_arithmetic<T>::value, "OHSHIT");
...
__forceinline static float wrap(float value, float min, float max) {
	return (float)fmod(fmod(value - min, max - min) + (max - min), max - min) + min;
}
__forceinline static float pingToSec(int ping) { return (float)ping / 1000.f; }
__forceinline static int tickToSec(int tick) { return (int)((float)tick / 20.f); }

__forceinline static std::string eraseDP(float num, int digits) { //erase decimal point
	std::string STUFF = std::to_string(num);
...
__forceinline static std::string secToMinStr(int second) {
	int sec = second % 60;
	std::string str = eraseDP((float)second / 60.f, 0) + ":" + ((sec <= 9) ? "0" : "") + std::to_string(sec);
...
struct vec2 {
	...
	std::string toString(int num) {
		return eraseDP(this->x, num) + ", " + eraseDP(this->y, num);
	}
...
struct vec3i {
	...
	vec3i below(int o) const {
		return vec3i(x, y - o, z);
	}

	vec3i above(int o) const {
		return vec3i(x, y + o, z);
	}
